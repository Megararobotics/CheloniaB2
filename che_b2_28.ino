...
#define Smoke_SENSOR_PIN A0  // Smoke sensor connected to analog pin A1 (Pin 28 in analogRead() is A1)
...
void setup() {
  ...
  Serial.println("Smoke detecting robot initialized. Monitoring Smoke levels...");
}
...
void loop() {
  ...
  if (SmokeValue > Smoke_THRESHOLD) {  // Check if Smoke level exceeds threshold
    ...
    moveForward();  // Move the robot forward if no Smoke detected
  }
